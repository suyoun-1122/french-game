...
let WORDS=[],LESSONS=[],currentLesson=1,currentWordIndex=0,wordFilter="all",wordSearch="";
...
let progress={version:"3.3",stars:0,today:todayKey(),todayDone:0,completedDesserts:[],words:{},totals:{attempts:0,correct:0},skillTotals:emptySkills(),bestCombo:0};
...
async function loadData(){
  const [w,l]=await Promise.all([fetch("./data/words.json?v=3.3").then(r=>r.json()),fetch("./data/lessons.json?v=3.3").then(r=>r.json())]);
